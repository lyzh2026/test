  ...
  return (
    <main className="min-h-screen p-8 animate-fade-in">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold" style={{ color: '#e8e9ed' }}>分发配置</h1>
          <p className="mt-1.5 text-sm" style={{ color: '#6b6d7b' }}>管理邮件与 Webhook 分发通道</p>
        </div>
      ...
      {loading ? (
        <p className="text-sm" style={{ color: '#6b6d7b' }}>加载中…</p>
      ) : items.length === 0 ? (
        <div className="card p-8 text-center text-sm" style={{ color: '#6b6d7b' }}>暂未配置分发通道</div>
      ) : (
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid #262933' }}>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: '#6b6d7b' }}>通道名称</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: '#6b6d7b' }}>类型</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: '#6b6d7b' }}>状态</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: '#6b6d7b' }}>创建时间</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: '#6b6d7b' }}>操作</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="transition-colors duration-150 hover:bg-surface-50" style={{ borderBottom: '1px solid #262933' }}>
                  <td className="px-4 py-3" style={{ color: '#e8e9ed' }}>{item.name}</td>
                  <td className="px-4 py-3 text-xs" style={{ color: '#a8abb8' }}>
                  ...
                  <td className="px-4 py-3 text-xs" style={{ color: '#6b6d7b' }}>
                    {item.created_at ? new Date(item.created_at).toLocaleString('zh-CN') : '-'}
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => handlePreview(item.channel_type)} className="text-xs transition-colors mr-3" style={{ color: '#a8abb8' }}>预览</button>
                    <button onClick={() => openEdit(item)} className="text-xs transition-colors mr-3" style={{ color: '#a8abb8' }}>编辑</button>
                    <button onClick={() => handleDelete(item.id)} className="text-xs" style={{ color: '#e45a5a' }}>删除</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in" style={{ background: 'rgba(0,0,0,0.6)' }}>
          <div className="w-full max-w-lg card p-6 animate-slide-up" style={{ background: '#0f1117' }}>
            <h2 className="mb-5 text-base font-medium" style={{ color: '#e8e9ed' }}>
              {editId ? '编辑' : '新增'}分发通道
            </h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs mb-1" style={{ color: '#6b6d7b' }}>通道类型</label>
              ...
              <div>
                <label className="block text-xs mb-1" style={{ color: '#6b6d7b' }}>通道名称</label>
              ...
              {formType === 'webhook' && (
                <div>
                  <label className="block text-xs mb-1" style={{ color: '#6b6d7b' }}>Webhook URL</label>
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer" style={{ color: '#a8abb8' }}>
                <input type="checkbox" checked={formEnabled} onChange={e => setFormEnabled(e.target.checked)}
                  className="h-4 w-4 rounded" style={{ accentColor: '#6b8cff' }} />
                启用
              </label>
            </div>
      ...
      {previewOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in" style={{ background: 'rgba(0,0,0,0.6)' }}>
          <div className="flex max-h-[80vh] w-full max-w-3xl flex-col card" style={{ background: '#0f1117' }}>
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #262933' }}>
              <h2 className="text-sm font-medium" style={{ color: '#e8e9ed' }}>
                周报预览 ({previewType === 'email' ? '邮件' : '飞书卡片'})
              </h2>
              <button onClick={() => setPreviewOpen(false)} className="text-lg leading-none" style={{ color: '#6b6d7b' }}>&times;</button>
            </div>
            <div className="flex-1 overflow-auto p-6">
              {previewLoading ? (
                ...
                <pre className="whitespace-pre-wrap break-all rounded p-4 text-xs font-mono" style={{ background: '#181b25', color: '#a8abb8' }}>
                  {previewContent}
                </pre>
              )}
            </div>
            <div className="flex justify-end px-6 py-3" style={{ borderTop: '1px solid #262933' }}>
