...
export default async function ArticlesPage({
  ...
  return (
    <main className="min-h-screen p-8 animate-fade-in">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold" style={{ color: '#e8e9ed' }}>文章列表</h1>
        <p className="mt-1.5 text-sm" style={{ color: '#6b6d7b' }}>浏览与管理采集的文章</p>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <label className="flex items-center gap-2 pb-0.5 text-xs text-text-secondary cursor-pointer">
            <input type="checkbox" name="bookmarked" value="1" defaultChecked={searchParams.bookmarked === '1'}
              className="h-4 w-4 rounded border-border-default" style={{ accentColor: '#6b8cff' }} />
            仅收藏
          </label>
