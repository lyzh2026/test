...
export default async function ArticleDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="min-h-screen p-8 animate-fade-in">
      <div className="mb-6">
        <Link href="/articles" className="text-sm transition-colors" style={{ color: '#a8abb8' }}>
          ← 返回文章列表
        </Link>
      </div>
      ...
      <article className="card p-8">
        <div className="mb-4 flex items-start gap-3">
          <h1 className="text-xl font-semibold flex-1" style={{ color: '#e8e9ed' }}>{article.original_title}</h1>
          <BookmarkButton articleId={article.id} initialBookmarked={article.bookmarked} />
        </div>

        <div className="mb-5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs" style={{ color: '#6b6d7b' }}>
        ...
        {article.ai.summary && (
          <section className="mb-6 rounded-[12px] p-5" style={{ background: '#181b25' }}>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider" style={{ color: '#6b6d7b' }}>AI 摘要</h2>
            <p className="text-sm leading-7" style={{ color: '#a8abb8' }}>{article.ai.summary}</p>
        ...
        {article.failed_reason && (
          <div className="mb-5 rounded-[12px] px-4 py-2.5 text-sm" style={{ background: 'rgba(228, 90, 90, 0.08)', color: '#e45a5a', border: '1px solid rgba(228, 90, 90, 0.15)' }}>
            失败原因：{article.failed_reason}
          </div>
        )}

        <div className="mb-6 flex items-center gap-4 text-sm">
          <a href={article.original_link} target="_blank" rel="noreferrer" className="transition-colors" style={{ color: '#6b8cff' }}>
            查看原文 ↗
          </a>
        ...
        {article.raw_content && (
          <section>
            <h2 className="mb-3 text-sm font-medium" style={{ color: '#e8e9ed' }}>原始正文</h2>
            <MarkdownContent content={article.raw_content} />
          </section>
        )}

        <div className="mt-6 flex justify-between text-xs" style={{ color: '#6b6d7b' }}>
