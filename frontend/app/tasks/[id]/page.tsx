...
export default async function TaskDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="min-h-screen p-8 animate-fade-in">
      <div className="mb-6">
        <Link href="/tasks" className="text-sm transition-colors" style={{ color: '#a8abb8' }}>
          ← 返回任务列表
        </Link>
      </div>
      ...
      <div className="card p-6 mb-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-lg font-semibold" style={{ color: '#e8e9ed' }}>{task.task_name || task.id}</h1>
            <p className="mt-1 text-sm" style={{ color: '#6b6d7b' }}>{task.date_to ? `${task.target_date} ~ ${task.date_to}` : task.target_date} · 优先级 {task.priority}</p>
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-4 text-xs md:grid-cols-4" style={{ color: '#6b6d7b' }}>
      ...
      {task.failed_details.length > 0 && (
        <section className="card p-6 mb-6">
          <h2 className="mb-4 text-sm font-medium" style={{ color: '#e8e9ed' }}>失败详情</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-left text-xs" style={{ color: '#6b6d7b' }}>
              ...
              <tbody>
                {task.failed_details.map((d, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #262933' }}>
                    <td className="break-all py-2 pr-2 text-xs" style={{ color: '#a8abb8' }}>{d.url}</td>
                    <td className="py-2 pr-4" style={{ color: '#e45a5a' }}>{d.code}</td>
                    <td className="py-2 pr-4">{d.stage ? <span className={STAGE_BADGE[d.stage] || 'badge-gray'}>{d.stage}</span> : '-'}</td>
                    <td className="py-2 text-xs" style={{ color: '#a8abb8' }}>{d.reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="card p-6">
        <h2 className="mb-4 text-sm font-medium" style={{ color: '#e8e9ed' }}>URL 清单（{task.url_list.length}）</h2>
        <ul className="max-h-96 space-y-1 overflow-y-auto text-xs">
          {task.url_list.map((u, i) => (
            <li key={i} className="break-all" style={{ color: '#a8abb8' }}>
              <a href={u} target="_blank" rel="noreferrer" className="transition-colors" style={{ color: '#a8abb8' }}>
                {u}
              </a>
            </li>
          ))}
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="mb-0.5" style={{ color: '#6b6d7b' }}>{label}</div>
      <div className="break-all" style={{ color: '#a8abb8' }}>{value}</div>
    </div>
  );
}
