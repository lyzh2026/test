  ...
  return (
    <div className="card p-6">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-base font-medium" style={{ color: '#18181b' }}>分发配置</h2>
      ...
      <p className="text-xs mb-4" style={{ color: '#9ca3af' }}>
        管理邮件与 Webhook 分发通道，配置后可定时推送周报。
      </p>

      {loading ? (
        <p className="text-sm" style={{ color: '#9ca3af' }}>加载中…</p>
      ) : items.length === 0 ? (
        <div className="rounded-lg p-6 text-center text-sm" style={{ background: '#f9fafb', color: '#9ca3af' }}>暂未配置分发通道</div>
      ) : (
        <div className="overflow-hidden rounded-lg" style={{ border: '1px solid #e5e7eb' }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="transition-colors duration-150" style={{ borderBottom: '1px solid #e5e7eb' }}>
