...
export function TaskProgress({
  ...
  return (
    <div className="mt-6">
      <div className="mb-2 flex items-center justify-between text-xs" style={{ color: '#6b6d7b' }}>
        <span>
          进度 {done} / {prog.total}
          {prog.failed > 0 && <span className="ml-2" style={{ color: '#e45a5a' }}>失败 {prog.failed}</span>}
        </span>
        <span>{pct}%</span>
      </div>
      <div className="h-1.5 overflow-hidden rounded-full" style={{ background: '#262933' }}>
        <div
          className="h-full rounded-full transition-all duration-300"
          style={{ width: `${pct}%`, background: '#6b8cff' }}
        />
      </div>

      {prog.currentUrl && !isFinished && (
        <p className="mt-2 truncate text-xs" style={{ color: '#6b6d7b' }} title={prog.currentUrl}>
          正在抓取：{prog.currentUrl}
        </p>
      )}

      {prog.message && !isFinished && (
        <p className="mt-1 text-xs" style={{ color: '#6b6d7b' }}>{prog.message}</p>
      )}
    </div>
  );
}
