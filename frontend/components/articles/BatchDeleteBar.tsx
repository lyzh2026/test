    ...
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm" style={{ color: '#9ca3af' }}>共 {total} 条</span>
        </div>
        <div className="flex items-center gap-4">
          {selected.size > 0 && (
            <span className="text-sm" style={{ color: '#9ca3af' }}>已选 {selected.size} 条</span>
          )}
          <button
      ...
      <div className="space-y-3">
        ...
        {items.map((a) => (
          <article key={a.id} className="card p-5 card-hover transition-all duration-200 animate-slide-up">
            <div className="mb-3 flex items-center gap-2 text-xs" style={{ color: '#9ca3af' }}>
              <input
                type="checkbox"
            ...
            <div className="mb-2 flex items-start gap-3">
              <h2 className="text-base font-medium flex-1">
                <a href={`/articles/${a.id}`} className="transition-opacity" style={{ color: '#18181b' }}>
                  {a.original_title}
                </a>
              </h2>
              <button
                ...
                style={{ color: a.bookmarked ? '#f59e0b' : '#d1d5db' }}
      ...
      <div className="mt-8 flex items-center justify-between text-sm">
        ...
        <span style={{ color: '#9ca3af' }}>{offset + 1} - {Math.min(offset + limit, total)}</span>
        <a
          href={nextHref}
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setConfirmOpen(false)}>
          ...
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <p className="text-sm font-medium" style={{ color: '#18181b' }}>确认批量删除</p>
            <p className="text-sm mt-1 mb-5" style={{ color: '#9ca3af' }}>确定删除选中的 {selected.size} 篇文章？删除后不可恢复。</p>
